...
python3 scripts/filter_explicit_rels.py \
  ...
  --output data/twiconv/twiconv.pdtb_explicit_train.rels
```

```bash
python3 scripts/filter_explicit_rels.py \
  --input PATH_TO_TWICONV_RELS/twiconv.pdtb_dev.rels \
  --output data/twiconv/twiconv.pdtb_explicit_dev.rels
```

```bash
python3 scripts/filter_explicit_rels.py \
  --input PATH_TO_TWICONV_RELS/twiconv.pdtb_test.rels \
  --output data/twiconv/twiconv.pdtb_explicit_test.rels
```
...
```text
twiconv.pdtb_explicit_all_test.rels
```
